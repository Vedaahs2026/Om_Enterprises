...
import { Trash2, Plus, Minus, ShoppingBag, ArrowRight, Loader2, CreditCard, ShieldCheck, CheckCircle2, Scissors, Sparkles, MapPin, AlertTriangle, Truck, Star, Pencil, ClipboardList } from "lucide-react";
  ...
  if (items.length === 0 && !isCheckoutModalOpen && !isProcessingPayment) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center px-4">
        ...
        <h1 className="text-3xl font-bold text-brand mb-4">Your quote list is empty</h1>
        <p className="text-brand/60 mb-10 text-center max-w-md leading-relaxed">
          Looks like you haven't added any electrical products to your quote list yet. Explore our catalog to request pricing.
        </p>
        <Link 
          href="/" 
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-12">
      <div className="flex items-center space-x-4 mb-6">
        <h1 className="text-3xl font-bold text-brand">Quote Request List</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-4">
          <div className="bg-white rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-gray-100 sticky top-24">            <h2 className="text-sm font-bold mb-6 tracking-widest uppercase border-b border-gray-100 pb-4 text-brand">Quote Summary</h2>
            ...
            <div className="space-y-4 mb-8">
              ...
              <div className="flex justify-between items-center text-brand/60 pb-4 border-b border-gray-100">
                ...
                {isLoadingRates ? (
                  <div className="h-4 w-16 bg-brand/10 rounded animate-pulse"></div>
                ) : shippingCost !== null ? (
                  <div className="flex items-center space-x-1.5 animate-in fade-in duration-200">
                    <span className="text-brand font-bold text-sm">₹{shippingCost.toFixed(2)}</span>
                    <button 
                      onClick={() => setIsPincodeModalOpen(true)}
                      className="text-brand hover:text-brand-hover p-1 rounded transition-colors cursor-pointer"
                  ...
                  <button 
                    onClick={() => setIsPincodeModalOpen(true)}
                    className="text-xs font-bold text-brand hover:text-brand-hover hover:underline cursor-pointer animate-in fade-in duration-200"
                  >
                    Estimate Delivery
                  </button>
                )}
            ...
            <div className="mt-6 flex items-center justify-center space-x-3 opacity-40">
              ...
              <div className="text-[8px] text-brand uppercase tracking-[0.3em] font-black">Quote Review</div>
            </div>
          </div>
        </div>
      </div>
          ...
          >
                  ...
                  <div className="space-y-4 pt-4 border-t border-brand/5">
                    ...
                    {isLoadingRates ? (
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        <div
                          className="p-4 rounded-2xl border-2 border-brand bg-brand/[0.03] shadow-xs flex items-start justify-between"
                        >
                          ...
                          <div className="text-right">
                            <span className="text-xs font-bold text-brand">₹{shippingCost.toFixed(2)}</span>
                          </div>
                        </div>
                      </div>
                    ) : (
            ...
            {paymentStep === "processing" && (
              <div className="py-20 flex flex-col items-center justify-center text-center animate-in fade-in zoom-in duration-500">
                ...
                <h3 className="text-2xl font-bold text-brand mb-2 font-inter">Submitting Quote Request</h3>
                <p className="text-[10px] text-brand/40 font-black uppercase tracking-widest">Registering your quote in our system...</p>
              </div>
            )}
